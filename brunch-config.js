...
(function() {
  ...
  var S, Sites, aSite, i, len, ref, siteName, theResult, theSite;
  ...
  theResult = {
    ...
    paths: {
      ...
      watched: [`domains/${siteName}/payload-`, `domains/${siteName}/templates`, 'vendor', 'app']
    },
    conventions: {
      ignored: function(path) {
        return /\.c9|\.git/.test(path);
      },
    ...
    modules: {
      autoRequire: {
        ...
        css: [siteName, `${siteName}/payload-/run-time.css`],
        "css/vendor.css": ["normalize", "blaze", "ace-css", "basscss-grid", "bootstrap"]
      },
      nameCleaner: (path) => {
        var c;
        c = path.replace(/^app\//, '');
        c = c.replace(RegExp(`^assets/${siteName}`), '');
        c = c.replace(/^domains\//, '');
        c = c.replace(RegExp(`${siteName}[\\/]payload-`), 'payload-');
        c = c.replace(RegExp(`${siteName}[\\/]templates`), '');
        if (path.match("nothing to see here")) {
    ...
    files: {
      javascripts: {
        joinTo: {
          'assets/js/app.js': [/^app/, RegExp(`domains/${siteName}\\/payload-`)],
          'assets/js/vendor.js': function(f) {
            var pattern, result;
      ...
      stylesheets: {
        ...
        joinTo: {
          'assets/css/app.css': [/^app/, RegExp(`domains\\/${siteName}\\/payload-`)],
