...
publishAll = () ->
    ...
    publisher.stderr.on 'data', (data) ->
        console.log "PUBLISH ERROR::", data.toString()
...
publishSites = (paths)->
  ...
  x.on 'ready',(argu)->
    console.log "publishSite triggered on ready"
    if unready
      console.log "Initial publication of sites - #{argu}"
      unready=false
      publishAll paths
  x.on 'all',(wha,path)->
    console.log "publishSite triggered on all - #{wha}, path=#{path}"
    return if unready
    publishAll paths
